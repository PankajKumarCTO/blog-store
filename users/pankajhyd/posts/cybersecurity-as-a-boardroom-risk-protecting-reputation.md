...
```mermaid
graph TD
    Boardroom["Board of Directors & Executive C-Suite"] --> RiskMgmt["Cybersecurity Risk Governance Framework"]
    
    subgraph BankVaultDefense["The Bank Vault Security Layers"]
        RiskMgmt --> Layer1["1. Reinforced Steel Vault Doors<br/>(Encryption & Firewalls)"]
        RiskMgmt --> Layer2["2. Biometric Access Scanners<br/>(Multi-Factor Auth & Identity)"]
        RiskMgmt --> Layer3["3. 24/7 Armed Security Guards<br/>(SOC Monitoring & Incident Response)"]
    end
    
    BankVaultDefense --> ReputationalTrust["Protected Brand Reputation & Customer Trust"]
```
...
```mermaid
graph LR
    Hacker["Cyber Hackers"] -->|Social Engineering Phone Call| Helpdesk["IT Helpdesk Employee"]
    Helpdesk -->|Granted Reset Password| AdminAccess["Admin Credential Takeover"]
    AdminAccess -->|Shut Down Systems| Outage["$100 Million Revenue Loss & Broken Keycards"]
```
